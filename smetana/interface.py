...
def extract_id_from_filepath(filepath):
    ...
    if filename.endswith(".xml"):
        ...
        raise IOError(
            "Unrecognized extension in file {}. Valid extensions are .xml and .xml.gz".format(
                filename
            )
        )
...
def load_communities(models, communities, other, flavor):
    ...
    if communities is not None:
        ...
        comm_dict = OrderedDict(
            (name, group[1].tolist()) for name, group in df.groupby(0)
        )
    else:
    ...
    if other:
        ...
        if missing:
            print(
                "Warning: {} models in perturbation file missing.".format(len(missing))
            )
...
def load_media_db(filename, sep="\t", medium_col="medium", compound_col="compound"):
    ...
    media_db = (
        data[[medium_col, compound_col]].groupby(medium_col).agg(lambda x: list(x))
    )
...
def define_environment(
    medium, media_db, community, mode, aerobic, verbose, min_mol_weight, use_lp
):
    ...
    if medium:
        ...
        def fmt_func(x):
            return "R_EX_M_{}_e_pool".format(x)

        env = Environment.from_compounds(
            media_db[medium], fmt_func=fmt_func, max_uptake=max_uptake
        )
        medium_id = medium
    elif mode == "global":
...
def run_global(
    ...
    mip, extras = mip_score(
        community,
        environment=env,
        verbose=verbose,
        min_mol_weight=min_mol_weight,
        use_lp=use_lp,
        exclude=excluded_mets,
    )
...
def run_detailed(
    ...
    if not ignore_coupling:
        if verbose:
            print(
                "Running SCS for community {} on medium {}...".format(
                    comm_id, medium_id
                )
            )
    ...
    mus = mu_score(
        community, environment=env, verbose=verbose, min_mol_weight=min_mol_weight
    )
    ...
    pairs = [
        (org1, org2)
        for org1 in community.organisms
        for org2 in community.organisms
        if org1 != org2
    ]
...
def run_abiotic(
    ...
    if sense == "rm":
        ...
        env = Environment.from_compounds(
            medium, fmt_func=lambda x: f"R_EX_M_{x}_e_pool", max_uptake=max_uptake
        )

    if len(modified) < p:
        raise RuntimeError(
            "Insufficient compounds ({}) to perform ({}) perturbations.".format(
                len(modified), p
            )
        )
    ...
    if n == 0:
        ...
        if verbose:
            print(
                "Running {} systematic abiotic perturbations with 1 compound...".format(
                    n
                )
            )
    else:
        ...
        if verbose:
            print(
                "Running {} random abiotic perturbations with {} compounds...".format(
                    n, p
                )
            )
...
def run_biotic(
    ...
    if len(inserted) < p:
        raise RuntimeError(
            "Insufficient species ({}) to perform ({}) perturbations.".format(
                len(inserted), p
            )
        )
    ...
    if n == 0:
        ...
        if verbose:
            print(
                "Running {} systematic biotic perturbations with 1 species...".format(n)
            )
        ...
        if verbose:
            print(
                "Running {} random biotic perturbations with {} species...".format(n, p)
            )
    ...
    for i in range(n):
        ...
        comm_models = [
            model_cache.get_model(org_id, reset_id=True) for org_id in new_species
        ]
        new_community = Community(
            comm_id, comm_models, copy_models=False, create_biomass=False
        )
        entries = run_detailed(
            new_id,
            new_community,
...
def export_results(mode, output, data, debug_data, zeros):
    ...
    if mode == "global":
        ...
        if len(debug_data) > 0:
            df = pd.DataFrame(
                debug_data, columns=["community", "medium", "key1", "key2", "data"]
            )
            df.to_csv(output + ".debug", sep="\t", index=False)
...
def main(
    ...
    model_cache, comm_dict, other_models = load_communities(
        models, communities, other_models, flavor
    )

    other_mets = other if mode == "abiotic" or mode == "abiotic-rm" else None
    media, media_db, excluded_mets, other_mets = load_media(
        media, mediadb, exclude, other_mets
    )
    ...
    for comm_id, organisms in list(comm_dict.items()):
        ...
        comm_models = [
            model_cache.get_model(org_id, reset_id=True) for org_id in organisms
        ]
        community = Community(comm_id, comm_models, copy_models=False)

        for medium in media:

            medium_id, env = define_environment(
                medium,
                media_db,
                community,
                mode,
            ...
            if mode == "global":
                entries, debug_entries = run_global(
                    comm_id,
                    community,
                    organisms,
                    medium_id,
