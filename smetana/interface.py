...
def extract_id_from_filepath(filepath):
  ...
  if filename.endswith('.xml'):
    ...
    raise IOError(
        'Unrecognized extension in file {}. Valid extensions are .xml and .xml.gz'.format(filename))
...
def load_communities(models, communities, other, flavor):
  ...
  if communities is not None:
    ...
    comm_dict = OrderedDict((name, group[1].tolist())
...
def load_media_db(filename, sep='\t', medium_col='medium', compound_col='compound'):
  ...
  media_db = data[[medium_col, compound_col]].groupby(
      medium_col).agg(lambda x: list(x))
...
def define_environment(medium, media_db, community, mode, aerobic, verbose, min_mol_weight, use_lp):
  ...
  if medium:
    def fmt_func(x): return "R_EX_M_{}_e_pool".format(x)
...
def run_detailed(comm_id, community, medium_id, excluded_mets, env, verbose, min_mol_weight, ignore_coupling):
  ...
  for org1, org2 in pairs:
    ...
    for met in sorted(metabolites):
      ...
      smt_data.append((comm_id, medium_id, org1, org2, met,
                       scs_o1_o2, mus_o1_met, mps_o2_met, smt))
...
def run_abiotic(comm_id, sense, community, medium_id, excluded_mets, env, verbose, min_mol_weight, other_mets, n, p,
  ...
  if len(modified) < p:
    raise RuntimeError(
        "Insufficient compounds ({}) to perform ({}) perturbations.".format(len(modified), p))
  ...
  data = run_detailed(comm_id, community, medium_id, excluded_mets,
                      env, False, min_mol_weight, ignore_coupling)
...
def run_biotic(comm_id, community, medium_id, excluded_mets, env, verbose, min_mol_weight, other_models, model_cache,
  ...
  if len(inserted) < p:
    raise RuntimeError(
        "Insufficient species ({}) to perform ({}) perturbations.".format(len(inserted), p))
  ...
  for i in range(n):
    ...
    comm_models = [model_cache.get_model(
        org_id, reset_id=True) for org_id in new_species]
...
def export_results(mode, output, data, debug_data, zeros):
  ...
  if mode == "global":
    ...
    df = pd.DataFrame(
        data, columns=['community', 'medium', 'size', "mip", "mro"])
    df.to_csv(output, sep='\t', index=False)

    if len(debug_data) > 0:
      df = pd.DataFrame(debug_data, columns=[
                        'community', 'medium', 'key1', "key2", "data"])
      df.to_csv(output + 'debug', sep='\t', index=False)

  else:

    columns = ['community', 'medium', 'receiver',
               'donor', 'compound', 'scs', 'mus', 'mps', 'smetana']
...
def main(models, communities=None, mode=None, output=None, flavor=None, media=None, mediadb=None, aerobic=None,
  ...
  for comm_id, organisms in list(comm_dict.items()):
    ...
    comm_models = [model_cache.get_model(
        org_id, reset_id=True) for org_id in organisms]
